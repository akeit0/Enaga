...
public interface IRuntimeTextServices : IDisposable
{
    ...
    RuntimeCaretPosition GetCaretPosition(
        SceneTextStyle style,
        string text,
        float lineHeight,
        float maxWidth,
...
public sealed class RuntimeBackendServices : IDisposable
{
    ...
    private sealed class MissingRuntimeTextServices : IRuntimeTextServices
    {
        private static InvalidOperationException CreateException() =>
            new(
                "No runtime text services are configured. Pass a backend service bundle when creating the runtime host."
            );
    ...
    private sealed class MissingRuntimeImageResolver : IRuntimeImageResolver
    {
        public RuntimeImageResolveResult ResolveImage(string source)
        {
            throw new InvalidOperationException(
                "No runtime image resolver is configured. Pass a backend service bundle when creating the runtime host."
            );
        }
    }
}
