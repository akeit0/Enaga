...
public static partial class HtmlDocumentLoader
{
    ...
    {
        using var httpClientLease = httpClientOptions is null
            ? null
            : CreateHttpClient(httpClientOptions);
        var httpClient = httpClientLease ?? SharedHttpClient;
        var document = await ReadTextSourceAsync(
                documentSource,
                null,
                httpClient,
                cancellationToken
        ...
        await LoadLinkedStyleSheetsAsync(document, styleSheets, httpClient, cancellationToken)
            .ConfigureAwait(false);

        if (!string.IsNullOrWhiteSpace(styleSheetSource))
        {
            var explicitStyleSheet = await ReadTextSourceAsync(
                    styleSheetSource,
                    document,
                    httpClient,
                    cancellationToken
