...
internal static class FramebufferDirtyRectScaler
{
        ...
        SceneDamageRectBufferWriter buffer)
    {
        ...
        var safeLogicalSize = new Vector2D<int>(Math.Max(1, logicalSize.X), Math.Max(1, logicalSize.Y));
        var safeFramebufferSize = new Vector2D<int>(Math.Max(1, framebufferSize.X), Math.Max(1, framebufferSize.Y));
        ...
        foreach (var dirtyRect in dirtyRects)
        {
            ...
            var right = Math.Clamp((int)MathF.Ceiling((dirtyRect.X + dirtyRect.Width) * scaleX), 0, safeFramebufferSize.X);
            var bottom = Math.Clamp((int)MathF.Ceiling((dirtyRect.Y + dirtyRect.Height) * scaleY), 0, safeFramebufferSize.Y);
