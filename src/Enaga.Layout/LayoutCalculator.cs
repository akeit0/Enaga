...
public sealed class LayoutCalculator
{
    ...
    )
    {
        if (input.PerformsLayout && frames.Length < children.Length)
            throw new ArgumentException(
                "Frame buffer is smaller than child request count.",
                nameof(frames)
            );
        ...
        if (!input.PerformsLayout)
        {
            ...
            var measuredContentWidth =
                axis == LayoutAxis.Row ? measured.MainSize : measured.CrossSize;
            var measuredContentHeight =
                axis == LayoutAxis.Row ? measured.CrossSize : measured.MainSize;
            var resolvedWidth =
                input.KnownDimensions.Width ?? measuredContentWidth + style.Padding.Horizontal;
            var resolvedHeight =
                input.KnownDimensions.Height ?? measuredContentHeight + style.Padding.Vertical;
        ...
        return new LayoutOutput(
            ...
            new LayoutRect(
                0,
                0,
                Math.Max(width, contentRight + style.Padding.Right),
                Math.Max(height, contentBottom + style.Padding.Bottom)
            )
        );
    }
    ...
    )
    {
        ...
        Span<int> lineStarts =
            children.Length <= 256 ? stackalloc int[children.Length] : new int[children.Length];
        Span<int> lineEnds =
            children.Length <= 256 ? stackalloc int[children.Length] : new int[children.Length];
        Span<float> resolvedWidths =
            children.Length <= 256 ? stackalloc float[children.Length] : new float[children.Length];
        ...
        for (var index = 0; index < children.Length; index++)
            entries[index] = MeasureChild(
                ...
                innerHeight,
                alignItems,
                applyCrossStretch: !wraps
            );

        var lineCount = BuildLines(
            children,
            entries,
            isColumn,
            availableMainSize,
    ...
    )
    {
        ...
        for (var lineIndex = 0; lineIndex < lineCount; lineIndex++)
        {
            ...
            var autoMainMarginSize =
                lineMetrics.AutoMainMarginCount > 0 && !lineMetrics.HasFlexibleAdjustment
                    ? Math.Max(0, availableMainSize - lineMetrics.OccupiedMainSize)
                        / lineMetrics.AutoMainMarginCount
                    : 0;
            var (startMain, gapBetweenItems) =
                lineMetrics.AutoMainMarginCount > 0
                    ? (0, mainGap)
                    : ResolveJustifyLayout(
                        justifyContent,
                        justifyRemainingMainSize,
                        lineMetrics.ValidCount,
                        mainGap
                    );
            ...
            for (var index = lineStarts[lineIndex]; index < lineEnds[lineIndex]; index++)
            {
                ...
                var occupiedMainSize = ResolveOccupiedMainSize(
                    entry,
                    isColumn ? resolvedHeights[index] : resolvedWidths[index],
                    isColumn,
                    autoMainMarginSize
                );
                var crossSize = occupiedCrossSizes[index];
                var crossStart = ResolveCrossStart(
                    entry.CrossAlign,
                    lineAvailableCross,
                    crossSize,
                    isCrossAxisReversed
                ...
                if (HasAutoCrossMargins(entry, isColumn))
                {
                    ...
                    var fixedCrossEndMargin = isColumn
                        ? entry.AutoMarginRight
                            ? 0
                            : entry.MarginRight
                        : entry.AutoMarginBottom
                            ? 0
                            : entry.MarginBottom;
                    var remainingCross = Math.Max(
                        0,
                        lineAvailableCross
                            - itemCrossSize
                    ...
                    if (isColumn)
                    {
                        crossStartMargin =
                            ...
                            : entry.AutoMarginLeft ? fixedCrossStartMargin + remainingCross
                            : fixedCrossStartMargin;
                    }
                    else
                    {
                        crossStartMargin =
                            entry.AutoMarginTop && entry.AutoMarginBottom
                                ? fixedCrossStartMargin + remainingCross * 0.5f
                            : entry.AutoMarginTop ? fixedCrossStartMargin + remainingCross
                            : fixedCrossStartMargin;
                    }

                    crossStart = 0;
                }
                var mainStart = ResolveMainStart(
                    startMain,
                    cursor,
                    occupiedMainSize,
                    availableMainSize,
    ...
    )
    {
        return crossAlign switch
        {
            ...
            CrossAlignment.Start => isCrossAxisReversed
                ? Math.Max(0, availableCross - occupiedCrossSize)
                : 0,
            _ => isCrossAxisReversed ? Math.Max(0, availableCross - occupiedCrossSize) : 0,
        };
    }
    ...
    )
    {
        ...
        for (var index = 0; index < entries.Length; index++)
        {
            ...
            var occupiedMain = ResolveBaseOccupiedMainSize(
                children[index],
                entry,
                isColumn,
                availableMainSize
    ...
    )
    {
        ...
        for (var index = start; index < end; index++)
        {
            ...
            var baseMainSize = ResolveBaseMainSize(
                children[index],
                entry,
                isColumn,
                availableMainSize
        ...
        for (var index = start; index < end; index++)
        {
            ...
            var mainSize = ResolveFlexibleMainSize(
                baseMainSize,
                entry,
            ...
            if (!entry.IsSpacer)
            {
                var adjusted = MeasureFlexAdjustedSize(
                    child,
                    textServices,
                    resultWidth,
                    resultHeight
            ...
            occupiedMainSizes[index] = ResolveOccupiedMainSize(
                entry,
                isColumn ? resultHeight : resultWidth,
                isColumn
            );
            occupiedCrossSizes[index] = ResolveOccupiedCrossSize(
                entry,
                resultWidth,
                resultHeight,
                isColumn
            ...
            occupiedMainTotal +=
                occupiedMainTotal > 0 ? gap + occupiedMainSizes[index] : occupiedMainSizes[index];
    ...
    private static float ResolveMinimumMainSize(
    ...
    )
    {
        if (isColumn)
            return child.HasMinHeight
                ? Math.Max(
                    0,
                    LayoutBoxModel.ResolveOuterSize(
                        child.MinHeight,
                        child.IsMinHeightPercent,
                        availableMainSize,
    ...
    )
    {
        ...
        if (baseRemainingMainSize < 0 && entry.FlexShrink > 0 && totalShrinkWeight > 0)
        {
            ...
            return Math.Max(
                0,
                baseMainSize + baseRemainingMainSize * (shrinkWeight / totalShrinkWeight)
            );
        }
    ...
    )
    {
        var baseMainSize = child.HasFlexBasis
            ? Math.Max(
                ...
                LayoutBoxModel.ResolveOuterSize(
                    child.FlexBasis,
                    child.IsFlexBasisPercent,
                    availableMainSize,
                    isColumn ? child.VerticalContentInset : child.HorizontalContentInset,
                    child.BoxSizing
                )
            )
        ...
        if (isColumn)
        {
            ...
            if (child.HasMaxHeight)
                baseMainSize = Math.Min(
                    baseMainSize,
                    LayoutBoxModel.ResolveOuterSize(
                        child.MaxHeight,
                        child.IsMaxHeightPercent,
                        availableMainSize,
                        child.VerticalContentInset,
                        child.BoxSizing
                    )
    ...
    private static float ResolveMainStartMargin(
        in StackLayoutEntry entry,
        bool isColumn,
    ...
    private static float ResolveMainEndMargin(
        in StackLayoutEntry entry,
        bool isColumn,
        float autoMarginSize
    )
    ...
    private static float ResolveOccupiedMainSize(
        in StackLayoutEntry entry,
        float mainSize,
        bool isColumn,
    ...
    private static float ResolveOccupiedCrossSize(
        in StackLayoutEntry entry,
        float width,
        float height,
        bool isColumn
    ...
    {
        return ResolveOccupiedMainSize(
            entry,
            ResolveBaseMainSize(child, entry, isColumn, availableMainSize),
            isColumn
        );
    }
    ...
    )
    {
        ...
        if (!child.Wrap)
            return (
                width,
                Math.Max(height, textServices.MeasureLineHeight(CreateChildFont(child, 18, 400)))
            );
    ...
    private static (float Width, float Height) ApplySizeConstraints(
        in LayoutChildRequest child,
        float width,
        float height,
    ...
    )
    {
        ...
        width = LayoutBoxModel.ResolveOuterSize(
            width,
            false,
            innerWidth,
            child.HorizontalContentInset,
            child.HasWidth ? child.BoxSizing : BoxSizingMode.BorderBox
        );
        height = LayoutBoxModel.ResolveOuterSize(
            height,
            false,
            innerHeight,
            child.VerticalContentInset,
            child.HasHeight ? child.BoxSizing : BoxSizingMode.BorderBox
        );
        ...
        if (child.Kind == LayoutChildKind.Button)
        {
            ...
            height =
                height > 0 ? height : Math.Max(40, labelHeight + 16 + child.VerticalContentInset);
        }
            ...
            height =
                (
                    child.Wrap
                        ...
                        : textServices.MeasureLineHeight(textStyle.Font)
                ) + child.VerticalContentInset;
        }
        ...
        if (child.Kind == LayoutChildKind.Divider && (width <= 0 || height <= 0))
        {
            ...
            width =
                (child.Vertical ? thickness : Math.Max(0, child.Length))
                + child.HorizontalContentInset;
            height =
                (child.Vertical ? Math.Max(0, child.Length) : thickness)
                + child.VerticalContentInset;
        }
        ...
        return new StackLayoutEntry(
            Valid: true,
            IsSpacer: false,
            OffsetLeft: child.HasLeft
                ? LayoutValue.Resolve(child.Left, child.IsLeftPercent, innerWidth)
                : 0,
            OffsetTop: child.HasTop
                ? LayoutValue.Resolve(child.Top, child.IsTopPercent, innerHeight)
                : 0,
    ...
    private static CrossAlignment ResolveCrossAlign(
    ...
    {
        return isColumn ? child.HasWidth : child.HasHeight;
    }
    ...
    {
        var resolvedExplicitSize = LayoutValue.Resolve(
            explicitSize,
            explicitSizeIsPercent,
            referenceSize
        );
        if (LayoutValue.IsSet(explicitSize))
            return resolvedExplicitSize;

        var resolvedStartInset = LayoutValue.Resolve(
            startInset,
            startInsetIsPercent,
            referenceSize
        );
        ...
        if (LayoutValue.IsSet(endInset))
            return Math.Max(
                0,
                referenceSize
                    - (LayoutValue.IsSet(startInset) ? resolvedStartInset : 0)
                    - resolvedEndInset
            );
    ...
    )
    {
        ...
        public bool HasFlexibleAdjustment =>
            (BaseRemainingMainSize > 0 && TotalGrow > 0)
    ...
    {
        public static StackLayoutEntry Invalid =>
            new(
                false,
                false,
                0,
    ...
    private static SceneFont CreateChildFont(
        in LayoutChildRequest child,
        float defaultSize,
        int defaultWeight
    )
    {
        if (child.Font.Size > 0 && (child.Font.Weight > 0 || defaultWeight == 400))
        {
            return child.Font;
        }
