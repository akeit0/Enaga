...
internal sealed class SceneCommitPainter : IDisposable
{
    ...
    private readonly Dictionary<string, RuntimeShaderTemplate?> runtimeShaderTemplateCache = new(
        StringComparer.Ordinal
    );
    private readonly Dictionary<string, SKRuntimeEffect?> shaderEffectCache = new(
        StringComparer.Ordinal
    );
    ...
    private readonly Dictionary<
        EllipsizedLineCacheKey,
        TextInputMetrics.TextLineSpan
    > ellipsizedLineCache = new();
    ...
    private readonly Dictionary<ScrollContentPictureCacheKey, SKPicture> scrollContentPictureCache =
        new();
    private readonly Queue<ScrollContentPictureCacheKey> scrollContentPictureCacheOrder = new();
    private readonly Dictionary<
        SceneNodeId,
        ScrollContentPictureCacheKey
    > scrollContentPictureKeyByNode = new();
    private readonly Dictionary<ScrollContentPictureCacheKey, SKImage> scrollContentRasterCache =
        new();
    private readonly Queue<ScrollContentPictureCacheKey> scrollContentRasterCacheOrder = new();
    private readonly Dictionary<
        SceneNodeId,
        ScrollContentPictureCacheKey
    > scrollContentRasterKeyByNode = new();
    ...
    private readonly SKPaint clearPaint = new() { BlendMode = SKBlendMode.Clear };
    private readonly SKPaint textPaint = new() { IsAntialias = true };
    private readonly SKPaint shadowPaint = new() { IsAntialias = true, Style = SKPaintStyle.Fill };
    ...
    public SceneCommitPainter(TimeProvider? timeProvider)
        : this(
            new SkiaTextResources(),
            timeProvider ?? TimeProvider.System,
            ownsTextResources: true
        ) { }
    ...
    private SceneCommitPainter(
        SkiaTextResources textResources,
        TimeProvider timeProvider,
        bool ownsTextResources
    )
    {
        this.textResources =
            textResources ?? throw new ArgumentNullException(nameof(textResources));
    ...
    internal int ScrollContentPictureCacheCount =>
        scrollContentPictureCache.Count + scrollContentRasterCache.Count;
    ...
    public void Paint(
        ...
        TimeSpan elapsed,
        ReadOnlySpan<SceneDamageRect> dirtyRects = default
    )
    {
    ...
    private void PaintDirtyRectsDirect(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        TimeSpan elapsed,
    ...
    )
    {
        ...
        BeginRecordingStats();
        bypassScrollContentCacheForCurrentPaint = ShouldBypassScrollContentCacheForDirtyPaint(
            commit,
            dirtyRects
        );
        try
    ...
    private static bool ShouldBypassScrollContentCacheForDirtyPaint(
        SceneLayoutCommit commit,
        ReadOnlySpan<SceneDamageRect> dirtyRects
    )
    {
        ...
        var viewportArea = Math.Max(
            1L,
            (long)Math.Max(1, commit.Viewport.Width) * Math.Max(1, commit.Viewport.Height)
        );
        var dirtyArea = 0L;
    ...
    {
        if (
            !commit.Layout.TryGetValue(id, out var box)
    ...
    private void PaintScrollViewChildren(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        SceneNodeId id,
        SceneGraphNode node,
    ...
    )
    {
        ...
        {
            if (
                TryGetOrCreateScrollContentRasterTile(
                    commit,
                    id,
                    node,
                    box,
    ...
    )
    {
        ...
        if (
            ...
            || IsHostAnimatedRuntimeShader(box)
            || HasPositionedChild(commit, node)
        )
            return false;
    ...
    private SKPicture? GetOrCreateScrollContentPicture(
        SceneLayoutCommit commit,
        SceneNodeId id,
        SceneGraphNode node,
        SceneLayoutBox box
    )
    {
        ...
        var contentWidth = Math.Max(box.Width, box.ContentWidth);
        var contentHeight = Math.Max(box.Height, box.ContentHeight);
        var viewportPicture = ShouldUseViewportScrollContentPicture(
            box,
            contentWidth,
            contentHeight
        );
        var visibleContentRect = ResolveVisibleScrollContentRect(
            box,
            contentWidth,
            contentHeight,
            viewportPicture
        ...
        PaintChildren(
            ...
            commit,
            node.Children,
            viewportPicture ? recordingCanvas.LocalClipBounds : null
        );
        var containsPending = recordingContainsPendingImages;
        recordingContainsPendingImages = previousPending || containsPending;
    ...
    private static int ComputeScrollContentSignature(
        SceneLayoutCommit commit,
        SceneGraphNode scrollNode
    )
    {
    ...
    private static bool ShouldUseViewportScrollContentPicture(
        SceneLayoutBox box,
        float contentWidth,
        float contentHeight
    ) =>
    ...
    private static SKRect ResolveVisibleScrollContentRect(
        SceneLayoutBox box,
        float contentWidth,
        float contentHeight,
        bool viewportPicture
    )
    {
        if (!viewportPicture)
    ...
    private static SKRect ResolveScrollContentTileRect(
        SceneLayoutBox box,
        float contentWidth,
        float contentHeight
    )
    {
        ...
        var tileHeight = Math.Max(
            1,
            Math.Min(contentHeight, Math.Max(box.Height, box.Height * 3f))
        );
        ...
        var contentLeft = Math.Clamp(
            MathF.Floor(Math.Max(0, box.ScrollX) / stepX) * stepX,
            0,
            maxContentLeft
        );
        var contentTop = Math.Clamp(
            MathF.Floor(Math.Max(0, box.ScrollY) / stepY) * stepY,
            0,
            maxContentTop
        );
        return SKRect.Create(
            box.AbsLeft + contentLeft,
            box.AbsTop + contentTop,
            tileWidth,
            tileHeight
        );
    }

    private static void AddNodeListToSignature(
        SceneLayoutCommit commit,
        ReadOnlySpan<SceneNodeId> ids,
        ref HashCode hash
    )
    {
        hash.Add(ids.Length);
    ...
    private static void AddNodeToSignature(
        SceneLayoutCommit commit,
        SceneNodeId id,
        ref HashCode hash
    )
    ...
    private void PaintChildren(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        ReadOnlySpan<SceneNodeId> childIds
    ...
    private static bool ShouldCullChildSubtree(
        SceneLayoutCommit commit,
        SceneNodeId childId,
        SceneLayoutBox? childBox,
        SKRect? subtreeCullRect
    )
    {
    ...
    private static SceneLayoutBox ApplyPaintOverride(
        SceneLayoutCommit commit,
        SceneNodeId id,
        SceneLayoutBox box
    )
    ...
    private void PaintAnimatedShaderNodes(
        SKCanvas canvas,
        SceneLayoutCommit commit,
    ...
    private void PaintAnimatedShaderRoot(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        SceneNodeId id,
        float elapsedSeconds
    ...
    private void PaintAnimatedShaderSubtree(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        SceneNodeId id,
        float elapsedSeconds
    ...
    private void PaintAnimatedShaderChildren(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        ReadOnlySpan<SceneNodeId> childIds,
        float elapsedSeconds
    ...
    private bool TryCreateGradientShader(SceneLayoutBox box, out SKShader? shader)
    {
        ...
        var stops =
            spec.Stops is { Length: > 0 } && spec.Stops.Length == colors.Length ? spec.Stops : null;
    ...
    private bool TryCreateRuntimeShader(
        SceneLayoutBox box,
        out SKShader? shader,
    ...
    private RuntimeShaderTemplate BuildRuntimeShaderTemplate(
        SKRuntimeEffect effect,
        SceneRuntimeShader shader
    )
    {
        ...
        else
        {
            ...
            {
                if (
                    shader.HostTime && string.Equals(uniform.Name, "time", StringComparison.Ordinal)
                )
                    continue;
    ...
    private RuntimeShaderUniformBinding? CreateUniformBinding(
        SKRuntimeEffect effect,
        SceneRuntimeShaderUniform value
    )
    {
        ...
        switch (value.Kind)
        {
            ...
            case SceneRuntimeShaderUniformKind.Color:
                return RuntimeShaderUniformBinding.CreateColor(
                    value.Name,
                    ToColorF(ResolveColor(value.ColorValue, SKColors.White))
                );
            case SceneRuntimeShaderUniformKind.FloatArray:
    ...
    private static RuntimeShaderUniformBinding? CreateNumericArrayBinding(
        string name,
        float[]? values
    )
    {
        ...
        {
            case 2:
                return RuntimeShaderUniformBinding.CreatePoint(
                    name,
                    new SKPoint(values[0], values[1])
                );
            case 3:
                return RuntimeShaderUniformBinding.CreatePoint3(
                    name,
                    new SKPoint3(values[0], values[1], values[2])
                );
            default:
    ...
    private static void AddCachedUniform(
        SKRuntimeEffectUniforms uniforms,
        RuntimeShaderUniformBinding binding
    )
    {
        switch (binding.Kind)
        {
    ...
    private void DrawText(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        var lineHeight =
            text.LineHeight > 0
                ? text.LineHeight
                : textResources.TextMeasurer.MeasureLineHeight(textStyle.Font);
        var textWidth = Math.Max(0, geometry.Width - geometry.PaddingLeft - geometry.PaddingRight);
        var layout = textResources.InputMetrics.CreateLayout(
            textStyle,
            textPaint,
            text.TextContent,
            lineHeight,
        ...
        for (var lineIndex = 0; lineIndex < layout.Lines.Count; lineIndex++)
        {
            ...
            var underlineWidth =
                textStyle.Underline && !textStyle.WrapText
            ...
            DrawTextLine(
                ...
                geometry.AbsTop
                    + geometry.PaddingTop
                    + ResolveTextBaselineOffset(lineToDraw, textStyle, lineHeight)
    ...
    )
    {
        ...
        var ellipsisLayout = textResources.InputMetrics.CreateLayout(
            textStyle with
            {
        ...
        var ellipsizedLine = textResources
            .InputMetrics.CreateLayout(
                textStyle with
                {
                    TextOverflowEllipsis = false,
                },
    ...
    private float ResolveTextBaselineOffset(
        TextInputMetrics.TextLineSpan line,
        SceneTextStyle textStyle,
        float lineHeight
    )
    ...
    private EllipsizedLineCacheKey CreateEllipsizedLineCacheKey(
        SceneTextStyle textStyle,
        string text,
        float textWidth,
        float lineHeight
    ...
    private TextInputMetrics.TextLineSpan StoreEllipsizedLine(
        EllipsizedLineCacheKey key,
        TextInputMetrics.TextLineSpan line
    )
    {
    ...
    {
        while (
            ellipsizedLineCache.Count > ellipsizedLineCacheLimit
            && ellipsizedLineCacheOrder.Count > 0
        )
        {
    ...
    private void DrawTextInput(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        var composedValue = hasComposition
            ...
            : value;
        var showPlaceholder =
            value.Length == 0 && !hasComposition && !string.IsNullOrEmpty(input.PlaceholderText);
        ...
        var resolvedInputTextColor = ResolveTextInputTextColor(
            showPlaceholder,
        ...
        var textWidth = Math.Max(0, geometry.Width - geometry.PaddingLeft - geometry.PaddingRight);
        var displayLayout = textResources.InputMetrics.CreateLayout(
            textStyle,
            textPaint,
            displayText,
            lineHeight,
        ...
        var valueLayout = showPlaceholder
            ? null
            : textResources.InputMetrics.CreateLayout(
                textStyle,
                textPaint,
                value,
                lineHeight,
        ...
        if (!showPlaceholder && selectionStart != selectionEnd && valueLayout is not null)
        {
            ...
            foreach (
                var rect in textResources.InputMetrics.GetSelectionRects(
                    valueLayout,
                    selectionStart,
                    selectionEnd
                )
        ...
        if (!showPlaceholder && hasComposition)
        {
            fillPaint.Color = ResolveColor(
                input.CompositionUnderlineColor,
                resolvedInputTextColor.WithAlpha(190)
            );
            var compositionStart = Math.Clamp(input.CompositionStart, 0, composedValue.Length);
            var compositionEnd = Math.Clamp(
                input.CompositionStart + compositionText.Length,
                0,
            ...
            var selectedStart =
                compositionStart
                + Math.Clamp(input.CompositionSelectionStart, 0, compositionText.Length);
            ...
            if (selectedLength > 0)
            {
                var selectedEnd = Math.Clamp(
                    selectedStart + selectedLength,
                    compositionStart,
                    compositionEnd
                );
                fillPaint.Color = ResolveColor(
                    input.CompositionSelectionUnderlineColor,
        ...
        for (var lineIndex = 0; lineIndex < displayLayout.Lines.Count; lineIndex++)
        {
            ...
            DrawTextLine(
                ...
                line,
                textX,
                geometry.AbsTop + geometry.PaddingTop + textStyle.FontSize + lineIndex * lineHeight,
                textStyle,
                line.Width
            );
        }
        ...
        var caretIndex = hasComposition
            ? Math.Clamp(
                input.CompositionStart
                    + Math.Clamp(input.CompositionCursorOffset, 0, compositionText.Length),
                0,
                composedValue.Length
            )
            : input.CaretIndex;
    ...
    private void DrawRadioControl(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        canvas.DrawCircle(
            centerX,
            centerY,
            Math.Max(0.5f, radius - strokePaint.StrokeWidth * 0.5f),
            strokePaint
        );
    ...
    private void DrawImeIndicator(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        var indicatorTypeface = textResources.FontCatalog.ResolveTypefaceForText(
            null,
            600,
            trimmedIndicator
        );
        using var indicatorFont = SkiaFontSynthesis.CreateFont(
            indicatorTypeface,
            12,
            600,
            italic: false
        ...
        canvas.DrawRoundRect(
            SKRect.Create(badgeLeft, badgeTop, badgeWidth, badgeHeight),
            10,
            10,
            badgePaint
        );
    ...
    )
    {
        ...
        {
            canvas.DrawLine(
                destinationRect.Left + 12,
                destinationRect.Top + 12,
                destinationRect.Right - 12,
                destinationRect.Bottom - 12,
                strokePaint
            );
            canvas.DrawLine(
                destinationRect.Right - 12,
        ...
        else
        {
            ...
            insetRect.Inflate(
                -Math.Min(24, destinationRect.Width * 0.14f),
                -Math.Min(24, destinationRect.Height * 0.14f)
            );
            canvas.DrawRoundRect(
                insetRect,
                Math.Min(14, box.BorderRadius + 6),
                Math.Min(14, box.BorderRadius + 6),
                fillPaint
            );
        }
    ...
    private bool TryDrawPlaceholderImage(
        SKCanvas canvas,
        SceneLayoutBox box,
        SKRect destinationRect
    )
    ...
    private static void DrawResolvedImage(
        SKCanvas canvas,
        SkiaImageAsset asset,
        SKRect destinationRect,
        string? fit,
        float borderRadius
    )
    {
        ...
        var fittedRect = ExpandImageRectToAvoidSamplingGaps(
            CalculateImageRect(sourceRect, destinationRect, fit),
            destinationRect
        );
        ...
        if (borderRadius > 0)
            canvas.ClipRoundRect(
                new SKRoundRect(destinationRect, borderRadius, borderRadius),
                antialias: true
            );
        else
    ...
    private static void DrawRepeatedImage(
        SKCanvas canvas,
        SkiaImageAsset asset,
        SKRect destinationRect,
        float borderRadius,
    ...
    )
    {
        ...
        if (asset.RasterImage is not null)
        {
            ...
            using var shader = asset.RasterImage.ToShader(
    ...
    private void DrawVerticalScrollBar(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        canvas.DrawRect(
            box.AbsLeft + box.Width - box.ScrollBarWidth,
            box.AbsTop,
            box.ScrollBarWidth,
            box.Height,
            fillPaint
        ...
        canvas.DrawRoundRect(
            metrics.Value.ThumbRect,
            metrics.Value.Radius,
            metrics.Value.Radius,
            fillPaint
        );
    }

    public void PaintScrollBars(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        float viewportScale,
    ...
    private void PaintNestedScrollBars(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        SceneNodeId id,
    ...
    private void PaintTopLevelScrollBars(
        SKCanvas canvas,
        SceneLayoutCommit commit,
        SceneNodeId id,
        float viewportScale,
    ...
    )
    {
        ...
        {
            DrawPresentationVerticalScrollBar(
                canvas,
                box,
                viewportScale,
                presentationWidth,
            ...
            DrawPresentationHorizontalScrollBar(
                canvas,
                box,
                viewportScale,
                presentationWidth,
        ...
        for (var index = 0; index < node.Children.Length; index++)
            PaintTopLevelScrollBars(
                ...
                presentationWidth,
                presentationHeight,
                hasScrollAncestor || isScrollView
            );
    }

    private void DrawPresentationVerticalScrollBar(
        SKCanvas canvas,
        SceneLayoutBox box,
        float viewportScale,
        int presentationWidth,
    ...
    {
        var metrics = ResolvePresentationVerticalScrollBar(
            box,
            viewportScale,
            presentationWidth,
            presentationHeight
    ...
    private void DrawPresentationHorizontalScrollBar(
        SKCanvas canvas,
        SceneLayoutBox box,
        float viewportScale,
        int presentationWidth,
    ...
    {
        var metrics = ResolvePresentationHorizontalScrollBar(
            box,
            viewportScale,
            presentationWidth,
            presentationHeight
    ...
    private void DrawHorizontalScrollBar(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        canvas.DrawRect(
            box.AbsLeft,
            box.AbsTop + box.Height - box.ScrollBarWidth,
            box.Width,
            box.ScrollBarWidth,
            fillPaint
        );
    ...
    private void DrawBorder(SKCanvas canvas, SceneLayoutBox box, SKRect rect, float radius)
    {
        if (box.Border is { } border)
        {
            if (
                radius > 0
                && TryResolveUniformBorder(
                    border,
                    out var uniformWidth,
                    out var uniformStyle,
                    out var uniformColor
                )
            )
            {
    ...
    {
        if (width <= 0 || style == SceneBorderStyle.None || string.IsNullOrWhiteSpace(color))
        {
            return;
        }
    ...
    private static void ClipBox(SKCanvas canvas, SceneLayoutBox box)
    {
        ...
        if (
            ...
            && SceneScrollBarLayout.ResolveVerticalScrollBar(box) is not null
        )
            rect.Right = Math.Max(rect.Left, rect.Right - Math.Max(0, box.ScrollBarWidth));
        if (
            box.NodeKind == SceneNodeKind.ScrollView
            && SceneScrollBarLayout.ResolveHorizontalScrollBar(box) is not null
        )
            rect.Bottom = Math.Max(rect.Top, rect.Bottom - Math.Max(0, box.ScrollBarWidth));
        if (geometry.FillRadius > 0)
        {
            canvas.ClipRoundRect(
                new SKRoundRect(rect, geometry.FillRadius, geometry.FillRadius),
                antialias: true
            );
            return;
        }
    ...
    private UnderlineStroke ResolveUnderlineStroke(
        TextInputMetrics.TextLineSpan line,
        SceneTextStyle textStyle
    )
    {
    ...
    private void DrawTextLineShadows(
        SKCanvas canvas,
        TextInputMetrics.TextLineSpan line,
        float x,
        float baselineY,
        SceneTextStyle textStyle,
        SceneBoxShadow[] shadows
    )
    {
    ...
    private SKTextBlob? GetOrCreateTextBlob(
        TextInputMetrics.TextRunSpan run,
        SceneTextStyle textStyle,
        SKColor textColor,
        out bool disposeBlob
    )
    {
        disposeBlob = false;
    ...
    private TextBlobCacheKey CreateTextBlobCacheKey(
        TextInputMetrics.TextRunSpan run,
        SceneTextStyle textStyle,
        SKColor textColor
    ) =>
    ...
    {
        while (
            scrollContentPictureCache.Count > scrollContentPictureCacheLimit
            && scrollContentPictureCacheOrder.Count > 0
        )
        {
            ...
            if (scrollContentPictureCache.Remove(oldestKey, out var oldest))
            {
                oldest.Dispose();
                if (
                    scrollContentPictureKeyByNode.TryGetValue(oldestKey.NodeId, out var currentKey)
    ...
    {
        while (
            scrollContentRasterCache.Count > scrollContentPictureCacheLimit
            && scrollContentRasterCacheOrder.Count > 0
        )
        {
            ...
            if (scrollContentRasterCache.Remove(oldestKey, out var oldest))
            {
                oldest.Dispose();
                if (
                    scrollContentRasterKeyByNode.TryGetValue(oldestKey.NodeId, out var currentKey)
                    && currentKey.Equals(oldestKey)
                )
                {
    ...
    {
        if (
            scrollContentPictureCacheOrder.Count <= Math.Max(16, scrollContentPictureCacheLimit * 2)
        )
            return;
    ...
    private static SKRect ExpandImageRectToAvoidSamplingGaps(
        SKRect fittedRect,
        SKRect destinationRect
    )
    {
    ...
    private static bool IsLineFullyVisible(
        float contentTop,
        float contentBottom,
        int lineIndex,
        float lineHeight
    ...
    internal static bool ShouldDrawTextLine(
        bool wrapText,
        float contentTop,
        float contentBottom,
        int lineIndex,
    ...
    internal static bool ShouldDrawTextCaret(
        float contentTop,
        float contentBottom,
        float caretTop,
        float caretHeight
    ...
    internal static SKColor ResolveTextInputTextColor(
    ...
    internal static SKColor ResolveTextInputFallbackColor(string? backgroundColor)
    {
        ...
        return luminance >= 0.62f ? new SKColor(0x11, 0x18, 0x27, 0xFF) : SKColors.White;
    }
    ...
    private static bool TryParseFunctionArguments(
        string color,
        string functionName,
        Span<Range> components,
        out ReadOnlySpan<char> arguments
    )
    {
        arguments = default;
    ...
    private static bool TryParseShortRgb(ReadOnlySpan<char> hex, out SKColor parsed)
    {
        parsed = default;
        if (
            !TryParseHexByte(hex[0], hex[0], out var red)
    ...
    private static bool TryParseRgb(ReadOnlySpan<char> hex, out SKColor parsed)
    {
        parsed = default;
        if (
            !TryParseHexByte(hex[0], hex[1], out var red)
    ...
    internal static PresentationScrollBarMetrics? ResolvePresentationVerticalScrollBar(
        SceneLayoutBox box,
        float viewportScale,
        int presentationWidth,
        int presentationHeight
    )
    {
        ...
        var right = Math.Min(
            Math.Max(0, presentationWidth),
            Snap((box.AbsLeft + box.Width) * scale)
        );
        ...
        var bottom = Math.Clamp(
            Snap((box.AbsTop + box.Height) * scale),
            0,
            Math.Max(0, presentationHeight)
        );
        ...
        var thumbHeight = Math.Min(
            ...
            Math.Max(
                ...
                trackHeight * Math.Min(1, viewportHeight / contentHeight)
            )
        );
        ...
        var thumbRect = new SKRect(
            gutterRect.Left + margin,
            thumbTop,
            gutterRect.Left + margin + barWidth,
            thumbTop + thumbHeight
        );
        return new PresentationScrollBarMetrics(gutterRect, thumbRect, barWidth * 0.5f);
    }

    internal static PresentationScrollBarMetrics? ResolvePresentationHorizontalScrollBar(
        SceneLayoutBox box,
        float viewportScale,
        int presentationWidth,
        int presentationHeight
    )
    {
        ...
        var thumbWidth = Math.Min(
            trackWidth,
            Math.Max(
                Math.Min(24, trackWidth),
                trackWidth * Math.Min(1, viewportWidth / contentWidth)
            )
        );
        ...
        var thumbRect = new SKRect(
            thumbLeft,
            gutterRect.Top + margin,
            thumbLeft + thumbWidth,
            gutterRect.Top + margin + barHeight
        );
    ...
    internal static bool TryHitVerticalScrollBarThumb(
        SceneLayoutBox box,
        float x,
        float y,
    ...
    {
        var resolved = SceneScrollBarLayout.TryHitVerticalScrollBarThumb(
            box,
            x,
            y,
            out var scrollBar,
    ...
    internal static float ResolveVerticalScrollOffsetFromThumbTop(
        SceneLayoutBox box,
        float thumbTop
    )
    {
    ...
    internal static bool TryHitHorizontalScrollBarThumb(
        SceneLayoutBox box,
        float x,
        float y,
    ...
    {
        var resolved = SceneScrollBarLayout.TryHitHorizontalScrollBarThumb(
            box,
            x,
            y,
            out var scrollBar,
    ...
    internal static float ResolveHorizontalScrollOffsetFromThumbLeft(
        SceneLayoutBox box,
        float thumbLeft
    )
    {
    ...
    internal readonly record struct BoxPaintGeometry(
        SKRect FillRect,
        SKRect BorderRect,
        float FillRadius,
        float BorderRadius
    ...
    internal readonly record struct VerticalScrollBarMetrics(
        SKRect TrackRect,
        SKRect ThumbRect,
        float Radius
    );

    internal readonly record struct HorizontalScrollBarMetrics(
        SKRect TrackRect,
        SKRect ThumbRect,
        float Radius
    );

    internal readonly record struct PresentationScrollBarMetrics(
        SKRect GutterRect,
        SKRect ThumbRect,
        float Radius
    );
    ...
    private static void DrawVectorImage(
        SKCanvas canvas,
        SKPicture picture,
        SKRect sourceRect,
        SKRect destinationRect
    ...
    {
        public static RuntimeShaderUniformBinding CreateInt(string name, int value) =>
            new(name, RuntimeShaderUniformKind.Int, IntValue: value);

        public static RuntimeShaderUniformBinding CreateFloat(string name, float value) =>
            new(name, RuntimeShaderUniformKind.Float, FloatValue: value);

        public static RuntimeShaderUniformBinding CreateColor(string name, SKColorF value) =>
            new(name, RuntimeShaderUniformKind.Color, ColorValue: value);

        public static RuntimeShaderUniformBinding CreatePoint(string name, SKPoint value) =>
            new(name, RuntimeShaderUniformKind.Point, PointValue: value);

        public static RuntimeShaderUniformBinding CreatePoint3(string name, SKPoint3 value) =>
            new(name, RuntimeShaderUniformKind.Point3, Point3Value: value);

        public static RuntimeShaderUniformBinding CreateFloatArray(string name, float[] value) =>
            new(name, RuntimeShaderUniformKind.FloatArray, FloatArrayValue: value);
    }
