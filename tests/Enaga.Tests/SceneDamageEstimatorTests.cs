...
public sealed class SceneDamageEstimatorTests
{
    ...
    private static SceneLayoutCommit CreateCommit(
        bool includeTooltip,
    ...
    )
    {
        var nodes = new Dictionary<SceneNodeId, SceneGraphNode>
        {
            [Root] = new(
                SceneNodeKind.View,
                null,
                includeTooltip ? [Sidebar, Content, Tooltip] : [Sidebar, Content]
            ),
        ...
        return new SceneLayoutCommit(Root, new SceneViewport(800, 600), nodes, layout, []);
    }
    ...
    private static SceneLayoutCommit CreateAnimatedCommit(bool includeTooltip)
    {
        ...
        return new SceneLayoutCommit(Root, new SceneViewport(800, 600), nodes, layout, [Shader]);
    }

    private static bool Contains(
        ReadOnlySpan<SceneDamageRect> dirtyRects,
        Func<SceneDamageRect, bool> predicate
    )
    {
